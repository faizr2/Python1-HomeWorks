...
# 2. Решение без re
# print(ord('A'), ord('Z'))
list_letters = list(map(lambda x: chr(x), list(range(65,91)))) # Преобразуем список из кодов ANSI в список букв A-Z
line_new = list(line)
 
for i, elem in enumerate(line_new[:]):
    for elem_2 in list_letters:
        if elem == elem_2:
            line_new[i] = ' '
